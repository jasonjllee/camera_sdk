...
namespace librealsense
{
    ...
    void goriila_s01_device::create_color_sensor()
    {
        ...
        color_ep->register_option(RS2_OPTION_GAIN, std::make_shared<platform::uvc_pu_option>(*color_ep, RS2_OPTION_GAIN));
        color_ep->register_option(RS2_OPTION_WHITE_BALANCE, std::make_shared<platform::uvc_pu_option>(*color_ep, RS2_OPTION_WHITE_BALANCE));
    ...
    std::shared_ptr<matcher> goriila_s01_device::create_matcher(const frame_holder& frame) const
    {
        ...
        if(_color_stream)
            streams.push_back(_color_stream.get());
